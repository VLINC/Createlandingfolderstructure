...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      const role = (
        userSnap.exists() ? (userSnap.data() as any).role : undefined
      ...
      // Customer/provider dashboard routing happens at /dashboard
      navigate("/dashboard", { replace: true });
    } catch (error: any) {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      // Keep messaging user-friendly; Firebase also returns auth/invalid-email, etc.
    ...
    <div className="pt-24 min-h-screen bg-gray-50 px-6 py-12">
      <div className="max-w-md mx-auto">
        ...
        <div className="bg-white p-8 rounded-[1.25rem] shadow-md">
            ...
            <div>
              ...
              <div className="mt-2 text-right">
                <button
                  ...
                  disabled={loading || resetLoading}
                  className="text-sm font-semibold text-[#D91429] hover:text-red-700 transition disabled:text-gray-400 disabled:cursor-not-allowed"
                >
          ...
          {/* Bottom signup links removed per request */}
        </div>
      </div>
    </div>
  );
